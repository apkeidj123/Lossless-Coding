/************************** Start of HUFF.C *************************
 *
 *      This is the Huffman coding module introduced in Chapter 5.
 *   Compile it with BITIO.C, ERRHAND.C, and either MAIN-C.C or MAIN-E.C
 */
...
void Hcount_bytes( FILE *input, unsigned long *long_counts );
 ...
 */
char *HCompressionName = "static order 0 model with Huffman coding";
 ...
 *
 */
void HCompressFile( FILE *input, BIT_FILE *output,int argc, char *argv[] )
{
 ...
 *
 */
void HExpandFile(BIT_FILE *input, FILE *output,int argc, char *argv[] )
{
    NODE *nodes;
    int root_node;
...
void Hcount_bytes(FILE *input, unsigned long *counts )
{
    long input_marker;
    int c;
