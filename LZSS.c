 ...
 * of between 2 and 17 bytes. It is introduced in Chapter 6.
 * Compile it with BITIO.C, ERRHAND.C and MAIN-C.C or MAIN-E.C
 */
...
#define END_OF_STREAMS        0         //change
#define UNUSEDS               0         //change
#define MOD_WINDOW( a )      ( ( a ) & ( WINDOW_SIZE - 1 ) )

char *LSCompressionName = "LZSS Encoder";
...
void InitTree( int r )
{
    ...
    tree[ r ].larger_child = UNUSEDS;
    tree[ r ].smaller_child = UNUSEDS;
}
...
int FindNextNode(int node )
{
    ...
    while ( tree[ next ].larger_child != UNUSEDS )
...
void DeleteString(int p )
{
    ...
    if ( tree[ p ].parent == UNUSEDS )
        return;
    if ( tree[ p ].larger_child == UNUSEDS )
        ContractNode( p, tree[ p ].smaller_child );
    else if ( tree[ p ].smaller_child == UNUSEDS )
    ...
    for ( ; ; ) {
        ...
        if ( *child == UNUSEDS ) {
            *child = new_node;
            ...
            tree[ new_node ].larger_child = UNUSEDS;
            tree[ new_node ].smaller_child = UNUSEDS;
            return( match_length );
        }
        test_node = *child;
    }
...
void LSCompressFile( FILE *input, BIT_FILE *output,int argc,char *argv[] )
{
    int i;
    int c;
    ...
    OutputBit( output, 0 );
    OutputBits( output, (unsigned long) END_OF_STREAMS, INDEX_BIT_COUNT );
    while ( argc-- > 0 )
...
void LSExpandFile( BIT_FILE *input, FILE *output,int argc, char *argv[] )
{
    int i;
    int current_position;
    int c;
    ...
    for ( ; ; ) {
        if ( InputBit( input ) ) {
            ...
            if ( match_position == END_OF_STREAMS )
                break;
